              ...
              <div className="bg-slate-50 dark:bg-slate-800 px-4 py-6 sm:px-6 border-b border-slate-200 dark:border-slate-700">
                <div className="flex items-center justify-between">
                  ...
                  <div className="ml-3 flex h-7 items-center">
                    <button
                      type="button"
                      className="rounded-md bg-white dark:bg-slate-800 text-slate-400 hover:text-slate-500 focus:outline-none focus:ring-2 focus:ring-lime-500"
                      onClick={toggleHistory}
                    >
              ...
              <div className="relative flex-1 px-4 py-6 sm:px-6">
                {history.length === 0 ? (
                  <div className="text-center text-slate-500 dark:text-slate-400 mt-10">
                    ...
                    <p className="text-sm mt-2">Utiliza el botón "Guardar" en las herramientas para registrar tus resultados aquí.</p>
                  </div>
                ) : (
                      ...
                      <li key={item.id} className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 border border-slate-200 dark:border-slate-700 relative group">
                         ...
                         <div className="space-y-2 text-sm font-mono">
                            <div className="bg-white dark:bg-slate-900 p-2 rounded border border-slate-200 dark:border-slate-700 flex justify-between items-center gap-2">
                                ...
                                <button onClick={() => handleCopy(item.input)} className="text-slate-400 hover:text-lime-500" title="Copiar entrada">
                                    <CopyIcon className="w-4 h-4" />
                                </button>
                            </div>
                            <div className="flex justify-center text-slate-400">↓</div>
                            <div className="bg-lime-50 dark:bg-slate-900 p-2 rounded border border-lime-200 dark:border-slate-700 flex justify-between items-center gap-2">
                                <span className="text-slate-800 dark:text-slate-200 break-all">{item.output}</span>
                                <button onClick={() => handleCopy(item.output)} className="text-slate-400 hover:text-lime-500" title="Copiar resultado">
                                    <CopyIcon className="w-4 h-4" />
                                </button>
                            </div>
                         </div>
